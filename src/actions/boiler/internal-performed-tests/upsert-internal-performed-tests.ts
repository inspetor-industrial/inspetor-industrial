...
export const upsertInternalPerformedTestsAction = authProcedure
  .createServerAction()
  .input(
    z.object({
  ...
  .handler(async ({ input, ctx }) => {
    try {
      ...
      await prisma.internalPerformedTests.upsert({
        where: {
      ...
      return returnsDefaultActionMessage({
        message: 'Dados dos testes internos salvos com sucesso',
        success: true,
      })
    } catch (error) {
      return returnsDefaultActionMessage({
        message:
          'Erro ao salvar dados dos testes internos, ' +
