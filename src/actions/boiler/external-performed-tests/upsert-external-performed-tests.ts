...
export const upsertExternalPerformedTestsAction = authProcedure
  .createServerAction()
  .input(
    z.object({
  ...
  .handler(async ({ input, ctx }) => {
    try {
      ...
      await prisma.externalPerformedTests.upsert({
        where: {
      ...
      return returnsDefaultActionMessage({
        message: 'Dados dos testes externos salvos com sucesso',
        success: true,
      })
    } catch (error) {
      return returnsDefaultActionMessage({
        message:
          'Erro ao salvar dados dos testes externos, ' +
