...
export const upsertLocalInstallationPerformedTestsAction = authProcedure
  .createServerAction()
  .input(
    z.object({
  ...
  .handler(async ({ input, ctx }) => {
    try {
      ...
      await prisma.localInstallationPerformedTests.upsert({
        where: {
      ...
      return returnsDefaultActionMessage({
        message: 'Dados dos testes de instalação local salvos com sucesso',
        success: true,
      })
    } catch (error) {
      return returnsDefaultActionMessage({
        message:
          'Erro ao salvar dados dos testes de instalação local, ' +
