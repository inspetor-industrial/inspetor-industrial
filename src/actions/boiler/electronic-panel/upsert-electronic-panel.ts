...
export const upsertElectronicPanelAction = authProcedure
  .createServerAction()
  .input(
    z.object({
  ...
  .handler(async ({ input, ctx }) => {
    try {
      ...
      await prisma.electronicPanel.upsert({
        where: {
      ...
      return returnsDefaultActionMessage({
        message: 'Dados do painel eletrônico salvos com sucesso',
        success: true,
      })
    } catch (error) {
      return returnsDefaultActionMessage({
        message:
          'Erro ao salvar dados do painel eletrônico, ' +
