...
export const upsertDischargeSystemAction = authProcedure
  .createServerAction()
  .input(
    z.object({
  ...
  .handler(async ({ input, ctx }) => {
    try {
      ...
      await prisma.dischargeSystem.upsert({
        where: {
      ...
      return returnsDefaultActionMessage({
        message: 'Dados do sistema de descarga salvos com sucesso',
        success: true,
      })
    } catch (error) {
      return returnsDefaultActionMessage({
        message:
          'Erro ao salvar dados do sistema de descarga, ' +
