...
import { getElectronicPanelByBoilerReportIdAction } from '@inspetor/actions/boiler/electronic-panel/get-electronic-panel-by-boiler-report-id'
import { upsertElectronicPanelAction } from '@inspetor/actions/boiler/electronic-panel/upsert-electronic-panel'
...
import { nrsForElectronicPanel } from '@inspetor/constants/nrs-electronic-panel'
...
const defaultQuestions: BoilerReportQuestion[] = electricalControlQuestions.map(
  (q) => ({
    question: q.question,
...
const defaultTests = {
  ...
  nrs: nrsForElectronicPanel.map((nr) => ({
    parent: nr.parent,
...
const electronicPanelSchema = z.object({
...
type ElectronicPanelFormValues = z.infer<typeof electronicPanelSchema>

type ElectronicPanelFormProps = {
  boilerId: string
  action?: 'view' | 'edit'
}

export function ElectronicPanelForm({
  boilerId,
  action = 'view',
}: ElectronicPanelFormProps) {
  const router = useRouter()
  const isViewMode = action === 'view'
  const [nrModalOpen, setNrModalOpen] = useState(false)

  const form = useForm<ElectronicPanelFormValues>({
    resolver: zodResolver(electronicPanelSchema),
    defaultValues: {
      observations: '',
      tests: defaultTests,
    },
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [result] = await getElectronicPanelByBoilerReportIdAction({
          boilerReportId: boilerId,
        })

        if (!result.success) {
          toast.error('Erro ao carregar dados do painel eletrônico')
          return
        }
  ...
  const onSubmit = async (values: ElectronicPanelFormValues) => {
    try {
      const [result] = await upsertElectronicPanelAction({
        boilerReportId: boilerId,
        tests: values.tests,
      ...
      toast.error('Erro ao salvar dados do painel eletrônico')
    }
  }
                ...
                render={({ field }) => (
                  <FormItem>
                    <div className="flex items-center justify-between">
                      <FormLabel>Painel Eletrônico</FormLabel>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
